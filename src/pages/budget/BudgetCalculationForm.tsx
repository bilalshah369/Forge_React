...
interface BudgetCalculationFormProps {
  ...
  isPopup?: boolean;
  //onClose?: () => void;
  onSubmit?: (data: any) => void; // Optional: To pass form data
...
const BudgetCalculationForm: React.FC<BudgetCalculationFormProps> = ({
  isPopup = false,
  projectId,
  projectName,
  onClose,
  ...
  const validateFields = () => {
    ...
    if (!newRow.total || newRow.total <= 0) {
  ...
  const handleEdit = async (data: BudgetRow) => {
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  ...
                  <input
                    ...
                    value={newRow.qty === 0 ? "" : newRow.qty}
                    //onChange={handleChange}
                    onChange={(e) => {
                      setNewRow({
                        ...newRow,
                        qty: parseFloat(e.target.value) || 0,
                        total: parseFloat(e.target.value) * newRow.value || 0,
                      });
                ...
                <div>
                  ...
                  <input
                    type="number"
                    name="costPerHour"
                    value={newRow.value === 0 ? "" : newRow.value}
                    readOnly={disableEdit}
                    //onChange={handleChange}
                    onChange={(e) => {
                      setNewRow({
                        ...
                        total: newRow.qty * parseFloat(e.target.value) || 0,
                      });
                ...
                <div>
                  ...
                  <input
                    ...
                    name="total"
                    //  value={newRow.total}
                    value={newRow.total === 0 ? "" : newRow.total}
                    //readOnly={disableEdit}
                    //onChange={handleChange}
                    onChange={(e) => {
                      //debugger;
              ...
              <div className="flex justify-end gap-2">
                <button
                  type="button"
                  onClick={() =>
                    onClose(totals.totalBudget, rows, "close", false)
                  }
                  className="bg-gray-300 text-black px-4 py-2 rounded"
                >
                  Cancel
...
export default BudgetCalculationForm;
